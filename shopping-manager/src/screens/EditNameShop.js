...
import { updateName, updateNameShop } from '../redux/auth/action'

const EditNameScreen = ({ route, navigation, auth }) => {
  const [name, setName] = useState(auth.shop?.name)
  const dispatch = useDispatch()
  const onSave = () => {
    dispatch(updateNameShop(name))
    goBack()
  }
  return (
    <SafeAreaView style={{ flex: 1 }}>
      ...
      <View style={styles.containerFull}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <Text style={styles.titleHeader}>Tên shop</Text>
