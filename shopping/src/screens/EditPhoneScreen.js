...
import { updatPhone } from '../redux/auth/action'
import { set } from 'react-native-reanimated';

var vnf_regex = /((09|03|07|08|05)+([0-9]{8})\b)/g;


const EditPhoneScreen = ({ route, navigation, auth }) => {
  ...
  const onSave = () => {
    if (!error) {
      dispatch(updatPhone(auth.user.id, phone))
      goBack()
    }
    // if (phone !== '') {
...
export default connect(mapStateToProps)(EditPhoneScreen);
