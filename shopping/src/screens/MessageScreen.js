...
import { View, StyleSheet, RefreshControl, ScrollView, Image, ActivityIndicator, Text, Dimensions, TextInput, SafeAreaView, useWindowDimensions } from 'react-native';
    ...
    useEffect(() => {
        ...
        async function fetchMyAPI() {
            ...
            await firestore()
                ...
                .then(data => {
                    data.forEach(documentSnapshot => {
                        setPartnerKey(documentSnapshot.id)
                        partnerKey = documentSnapshot.id
                    });
                })
                ...
                <View style={styles.container}>
                    <ScrollView showsVerticalScrollIndicator={false}
                        ...
                        {messageData.map((item, index) =>
                            item.ownerId == auth.user.id ? <ItemRight item={item} key={index} /> : <ItemLeft item={item} key={index} shop={shop} />)}
